...
public class EggplantTests
{
    ...
    [Fact]
    public void PlayShouldMoveTwoCardOfEachPlayerToThePlayerToTheirLeft()
    {
        Player player1 = new Player("Piet/a/0/0/0/1|Joop/ppppp/0/0/0/0|Jan/rrr/0/0/0/0|Jaap/ccccc/0/0/0/0|0|0");
        player1.Hand.Add(new Eggplant());
        player1.Hand[5].Play(player1, Array.Empty<string>());
        Assert.Equal(2,player1.Hand.Count);
        Assert.Equal(4,player1.GetPlayerByName("Jaap").Hand.Count);
        Assert.True(player1.Hand.OfType<Potato>().Count()==2);
        Assert.True(player1.GetPlayerByName("Joop").Hand.OfType<Rubarb>().Count()==2);
        Assert.True(player1.GetPlayerByName("Jan").Hand.OfType<Carrot>().Count()==2);
        Assert.True(player1.GetPlayerByName("Jaap").Hand.OfType<Artichoke>().Count()==1);
    }
}